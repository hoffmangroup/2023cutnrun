...
#This script is modified from https://github.com/Henikoff/Cut-and-Run/blob/master/spike_in_calibration.csh

#Seven arguments
#                   1           2            3        4                  5             6      
#spike_calibrate.csh genome.bed scale output(bg|bga|d) genome_chr_lens_file  min_len max_len
...
if ($#argv < 6) then
	...
	echo "scale is 1 for sample with fewer spike-in reads and (smaller spike-in count / larger one) for sample with more spike-in reads"
...
#Select fragments within the length range, assumes fragment length the final column of the BED file
awk -v min=$min_len -v max=$max_len '{if ($NF >= min && $NF <= max) print}' $genome_bed > $$.temp.bed
